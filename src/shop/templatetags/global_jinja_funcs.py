...
def get_shop_product_types():
    ...
    if not product_types_infos:
        for product_type in models.ProductType.objects.all().filter(show_in_menu=True).order_by('name'):
...
def get_shop_designs():
    ...
    if not designs_infos:
        for design in models.Design.objects.all().filter(show_in_menu=True).order_by('name'):
